...
Historical accepted design from PR #221. Its dated operational figures and
ceiling statements record that earlier gate only; they are not a current
checkpoint, authorization or instruction to open an operator ledger. The
later bound-v2 accounting integration is specified in
`bound-embedding-accounting.md` and likewise grants no provider access.
