...
The retained historical offline diagnostic isolates candidate visibility in the public core at
...
coverage states, lexical scores and local timings. Its bytes remain frozen at
SHA-256 `917068fadc720d665f167efb6190ab2ef8ec722fdcf74611275d80b6685da583`.
Run the diagnostic against the **current checked-out runtime**, not a reproduction
of that historical runtime, with Node 22.16 or 24:
...
public inventory placement independent of UUID ordering. Do not overwrite the
retained historical JSON with this command's output. The experiment does not load user
databases or credentials and makes zero provider requests.

## Historical observations at runtime base 5dbf73c
...
The diagnostic imports the checked-out core, so later catalog changes change
that observation; the retained JSON above preserves the baseline outcome. Independent review added
...
## Current integration regression contract

The integrated query-aware candidate path no longer equates public inventory
page order with recall visibility. Tests now require every target in the four
late/misfiled/unfiled/multiple-evidence controls to be visible and recalled by the
bounded oracle, while their public inventory pages still remain beyond page two.
Large cases still report `budget_exhausted`; reaching their requested targets
does not imply complete traversal. The current topic-only classification catalog
must be empty and complete for the 101-unfiled-memory control, and the proposal
must succeed. A separate test fixes the historical report hash and checks its
original zero-target misses and rejected classification result.

These are exact current-runtime regression expectations, not permissive
old-or-new outcomes or a rewrite of the historical experiment. The unchanged
helper, fixtures, lexical control and constant-one counter still measure bounded
visibility with a target-ID oracle, not semantic retrieval, user benefit or
provider cost. No new model calls or refreshed historical result are implied.
