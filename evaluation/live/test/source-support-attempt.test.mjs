...
import { createSourceSupportAttempt, SOURCE_SUPPORT_LIMITS, createCandidateQualificationAttempt,
  createQualificationPilotAttempt, CANDIDATE_QUALIFICATION_LIMITS, QUALIFICATION_PILOT_LIMITS } from '../qualification-pilot-attempt.mjs';
const body = (method = 'rank') => JSON.stringify({ model: 'gpt-4.1-mini-2025-04-14', text: { format: { name: `cairn_${method}` } } });
function fixture(overrides = {}, factory = createSourceSupportAttempt) {
...
test('SS1 closed 96/$0.48 ceiling serializes mixed count/generation requests before permanently rejecting 97', async () => {
  assert.deepEqual(SOURCE_SUPPORT_LIMITS, { requests: 96, microUsd: 480000, reservationMicroUsd: 5000 });
  const f = fixture(); const methods = ['extract', 'qualifyCandidates', 'classify', 'select', 'rank'];
  await Promise.all(Array.from({ length: 96 }, (_, i) => f.gate.request(i % 2 ? '/responses' : '/responses/input_tokens', body(methods[i % 5]))));
  assert.equal(f.sends(), 96); assert.equal(f.peak(), 1); assert.equal(f.gate.getState().reservedMicroUsd, 480000);
  await assert.rejects(f.gate.request('/responses', body())); assert.equal(f.sends(), 96); assert.ok(f.gate.getState().halted);
});
test('SS2 older factories retain caps and reject select/rank; new factory rejects unrelated methods/models', async () => {
...
test('SS3 exact options/checkpoint and conservative headroom cannot be widened by callers', () => {
  const f = fixture();
  for (const extra of [{ limits: { requests: 1000 } }, { methods: ['judge'] }]) assert.throws(() => createSourceSupportAttempt({ ...f.config, ...extra }));
  for (const patch of [{ requestCap: 100 }, { limitMicroUsd: 504999 }, { state: 'closed' }, { requestCount: 6 },
    { attempts: [{ outcome: null }] }]) assert.throws(() => createSourceSupportAttempt({ ...f.config, readState: () => ({ ...f.ledger, ...patch }) }));
  assert.throws(() => createSourceSupportAttempt({ ...f.config, expectedCheckpoint: { ...f.config.expectedCheckpoint, extra: true } }));
});
test('SS4 pin/persistence/accessor/accounting/unsettled failures halt permanently with no sends', async () => {
...
test('SS5 failed late transport halts queued requests without refund, readonly violation never resumes', async () => {
