...
const namespace = { ownerId: 'index-upgrade', scope: 'personal', projectId: null };
const timestamp = '2025-01-01T00:00:00.000Z';
const tables = ['memories', 'receipts', 'suppressed', 'namespace_epochs', 'store_metadata',
  'mocs', 'moc_memory_refs', 'moc_edges', 'moc_title_sources', 'admission_claims', 'memory_conflicts'];
const snapshot = (db) => tables.map((table) => db.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all());
function fixture(t) {
  const path = join(mkdtempSync(join(tmpdir(), 'cairn-index-v6-')), 'memory.sqlite');
  closeSync(openSync(path, 'ax', 0o600));
  const db = new DatabaseSync(path); t.after(() => db.close());
  db.exec(readFileSync(new URL('../testing/index-schema-v6.sql', import.meta.url), 'utf8'));
  db.exec(`INSERT INTO store_metadata VALUES (1,'synthetic-store','${'ab'.repeat(32)}');
    INSERT INTO namespace_epochs VALUES ('index-upgrade','personal','',17);
    INSERT INTO suppressed VALUES ('index-upgrade','personal','','${'c'.repeat(64)}');`);
  for (const id of ['a', 'b']) {
    db.prepare(`INSERT INTO memories VALUES (?,'index-upgrade','personal','',?,?,'fact','explicit',1,3,0,?,?,'filed')`)
  ...
  db.prepare(`INSERT INTO mocs VALUES ('moc','index-upgrade','personal','',1,'Topic','topic',2,?,?)`).run(timestamp, timestamp);
  db.prepare(`INSERT INTO mocs VALUES ('parent','index-upgrade','personal','',2,'Parent','parent',1,?,?)`).run(timestamp, timestamp);
  db.exec(`INSERT INTO moc_memory_refs VALUES ('moc',2,'a',3);
    ...
    INSERT INTO admission_claims VALUES ('index-upgrade','personal','','synthetic','done','${'a'.repeat(64)}','completed',NULL,NULL,'["a"]',1);
    INSERT INTO admission_claims VALUES ('index-upgrade','personal','','synthetic','pending','${'b'.repeat(64)}','pending','synthetic-token',${Date.now() + 120000},NULL,NULL);`);
  db.exec("INSERT INTO memory_conflicts VALUES ('a',3,'b',3,'contradicts','explicit-hint')");
  return { path, db };
}
function cursor(db) {
...
test('R08 v6→v7 preserves all values, claims, placements, suppression and signed cursors', (t) => {
  ...
  assert.equal(core.get({ namespace, memoryId: 'a' }).value.conflicts.length, 1);
...
test('R08 v6 DDL collision preserves schema and state then retries', (t) => {
  const { path, db } = fixture(t);
  db.exec("CREATE TABLE index_generations(preserve_me TEXT); INSERT INTO index_generations VALUES ('survivor')");
...
test('R08 v6 writer contention leaves old schema intact and retryable', (t) => {
  const { path, db } = fixture(t);
  const before = snapshot(db);
  const schema = db.prepare('SELECT * FROM sqlite_master ORDER BY name').all();
